 ...
 */
void usage() {
  std::cout << "Usage: " << program_invocation_short_name << " --port=PORT --seeds=SEEDS_LIST \n"
    << "\t\t[--timeout=TIMEOUT] [--http [--http-port=HTTP_PORT]]\n"
    << "\t\t[--debug] [--version] [--help]\n\n"
    ...
    << "\t           the DNS-resolvable `hostname`, examples:\n\n"
    ...
    << "\t           between entries; the host must not ALL be active.\n"
    << "\t           Required.\n"
