...
public:
  ...
  typedef enum {LEFT, RIGHT} WhichNode;
  ...
  MerkleNode(const MerkleNode<T, hash_func, hash_len>& left,
    ...
    // In C++ we cannot invoke it here, even if this constructor is invoked via the constructor
    // of a derived class, which does implement `computeHash()`: at this point that object is not
    // yet constructed, and this invocation here would trigger a "pure virtual" function
    // invocation error.
    //    hash_ = computeHash();
...
const NodeType* build_(NodeType* nodes[], size_t len) {
  ...
  if (len == 1) return new NodeType(*nodes[0], *nodes[0]);
