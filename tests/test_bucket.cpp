...
TEST(BucketTests, Compare) {
    std::vector<float> points{0.3, 0.6, 0.9};
    Bucket b1("with_points", points);
    Bucket b2("with_points", points);
    ASSERT_TRUE(b1 == b2);
    Bucket b3("aardvark", points);
    ASSERT_FALSE(b1 < b3);
    std::vector<float> mo_points{0.3, 0.6, 0.8};
    Bucket b4("with_points", mo_points);
    ASSERT_TRUE(b4 < b1) << "same name, same number of points";
    mo_points.push_back(0.9);
    Bucket b5("with_points", mo_points);
    ASSERT_TRUE(b5 < b1) << "same name, different points";
}
...
TEST(BucketTests, CanFindNearest) {
    Bucket b("abucket", {
            0.0422193, 0.0592362, 0.119673, 0.215251, 0.90553});
...
TEST(BucketTests, ThrowsOutOfRange) {
    ...
    ASSERT_THROW(
            static_cast<void>(b.partition_point(b.partitions() + 2)),
            std::out_of_range);
}
